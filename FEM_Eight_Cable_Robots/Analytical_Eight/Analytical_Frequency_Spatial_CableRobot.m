function [f2,K1,M1,nodeCoordinates,elementNodes]=Analytical_Frequency_Spatial_CableRobot(E,w,A,n,T,Ax,Ay,Az,a,b,mass)
...
%     L1 = sqrt((x2-x1)^2 + (y2-y1)^2 +(z2-z1)^2);
    if e<n+1;
        [K,L]=TangentStiffness(E,A,w,I,J,T(1));
        % Stiffness Matrix
        stiffness(elementDof,elementDof)= stiffness(elementDof,elementDof)+K;
        %         indice
    elseif n+1<=e & e<2*n+1;
        [K,L]=TangentStiffness(E,A,w,I,J,T(2));
        % Stiffness Matrix
        stiffness(elementDof,elementDof)= stiffness(elementDof,elementDof)+K;
        %         indice
    elseif 2*n+1<=e & e<3*n+1;
        [K,L]=TangentStiffness(E,A,w,I,J,T(3));
        % Stiffness Matrix
        stiffness(elementDof,elementDof)= stiffness(elementDof,elementDof)+K;
        %         indice
    elseif 3*n+1<=e & e<4*n+1;
        %         indice
        [K,L]=TangentStiffness(E,A,w,I,J,T(4));
        % Stiffness Matrix
        stiffness(elementDof,elementDof)= stiffness(elementDof,elementDof)+K;
    elseif 4*n+1<=e & e<5*n+1;
        %         indice
        [K,L]=TangentStiffness(E,A,w,I,J,T(5));
        % Stiffness Matrix
        stiffness(elementDof,elementDof)= stiffness(elementDof,elementDof)+K;
    elseif 5*n+1<=e & e<6*n+1;
        %         indice
        [K,L]=TangentStiffness(E,A,w,I,J,T(6));
        % Stiffness Matrix
        stiffness(elementDof,elementDof)= stiffness(elementDof,elementDof)+K;
    elseif 6*n+1<=e & e<7*n+1;
        %         indice
        [K,L]=TangentStiffness(E,A,w,I,J,T(7));
        % Stiffness Matrix
        stiffness(elementDof,elementDof)= stiffness(elementDof,elementDof)+K;
    else 7*n+1<=e & e<8*n+1;
        %         indice
        [K,L]=TangentStiffness(E,A,w,I,J,T(8));
        % Stiffness Matrix
        stiffness(elementDof,elementDof)= stiffness(elementDof,elementDof)+K;
    end
    M(elementDof,elementDof)= M(elementDof,elementDof)+(w*L/(6*g))*[2*II, II;II, 2*II];
end
